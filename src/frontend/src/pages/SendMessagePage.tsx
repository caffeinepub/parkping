...
export default function SendMessagePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = (await (anonActor as any).createChatSession(
        recipientPrincipal,
      )) as { sessionId: bigint; senderToken: string };
      await (anonActor as any).sendSenderChatMessage(
        result.sessionId,
        result.senderToken,
        message.trim(),
        [],
      ...
      navigate({
        to: `/chat?session=${result.sessionId}&token=${result.senderToken}`,
      });
    } catch (e: any) {
  ...
  const displayName = recipientProfile?.displayName || "the vehicle owner";
